...
/*(10x10)*/    vector<vector<float>> bMatrix = {{2.5, 7.6, 2.3, -1.3, 0.7, -4.8, 3.1, 0, -6, -0.5}, {3.8, 0.2, -7.4, 5.6, -2.9, 6, 4, 8, -1.3, 8.5}, {4.5, 7.8, -2.3, -6.7, 1.2, 5.6, 3.4, -9, 0.1, 8}, {4.3, -7.5, 9.8, 2.5, -5.7, 8.9, 6.3, -3.1, 1.5, 0.6}, {-1, 3.2, 6.6, -8.8, 10, 2.2, 5.5, -7.7, 0, 4.4}, {8.7, -6.3, 4.9, 1.3, -5, 2.7, 0.9, 9.2, -7, 3}, {3.8, 0, 7.4, -5.6, 2.9, 9, 6.8, -4.7, 8.2, 1.3}, {2.4, 1.2, -3.6, 5.8, 4.1, 9.7, 6.5, -8.9, 7.3, -0.9}, {1.5, 2.7, 0, 5, -7.6, 8, 3, -4, 0, -8}, {2.3, 1, 6, 8.2, -4.5, 9, 6.9, 0, -6.2, 2}};
    ...
    // vector<vector<int8_t>> encode_plain1(aMatrix.size(), vector<int8_t>(aMatrix[0].size()));
    // for (int i = 0; i < aMatrix.size(); i++){
    //     for (int j = 0; j < aMatrix[0].size(); j++){
    //         encode_plain1[i][j] = (int8_t)(aMatrix[i][j]*pow(2,length/2));
    //     }
    // }
    vector<vector<int8_t>> encode_plain2(bMatrix.size(), vector<int8_t>(bMatrix[0].size()));
    for (int i = 0; i < bMatrix.size(); i++){
        for (int j = 0; j < bMatrix[0].size(); j++){
            encode_plain2[i][j] = (int8_t)(bMatrix[i][j]*pow(2,length/2));
        }
    }
    // vector<vector<int8_t>> encode_plain3(cMatrix.size(), vector<int8_t>(cMatrix[0].size()));
    // for (int i = 0; i < cMatrix.size(); i++){
    //     for (int j = 0; j < cMatrix[0].size(); j++){
    //         encode_plain3[i][j] = (int8_t)(cMatrix[i][j]*pow(2, length/2));
    //     }
    // }
    // vector<vector<int8_t>> encode_plain4(dMatrix.size(), vector<int8_t>(dMatrix[0].size()));
    // for (int i = 0; i < dMatrix.size(); i++){
    //     for (int j = 0; j < dMatrix[0].size(); j++){
    //         encode_plain4[i][j] = (int8_t)(dMatrix[i][j]*pow(2, length/2));
    //     }
    // }
    // vector<int8_t> encode_plain5(centering.size());
    // for (int i = 0; i < centering.size(); i++){
    //     encode_plain5[i] = (int8_t)(centering[i]*pow(2, length/2));
    // }
    // int8_t encode_plain6 = (int8_t)(number*pow(2, length/2));
