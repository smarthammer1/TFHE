...
int main() {
    ...
    // const int d = 2;
    // const int r = 1;
    const int number_of_iteration = 5;      // at least 3
    float num = 3.6;
    const int dimension = 4;                // at least 2

    // vector<vector<float>> aMatrix = {{1, 13, 4},{-3, -39, -12}, {1.6, 20.8, 6.4}};
    // vector<vector<float>> aMatrix = {{1.3, 0, 7.5}, {1, -3.6, -4}};
    vector<vector<float>> bMatrix = {{2.5, 3}, {4.1, -7}};
    // vector<vector<float>> aMatrix = {{2.5, 3}, {3, -4}};
    // vector<vector<float>> aMatrix = {{5, 6}, {7.1, -11}};
    // vector<vector<float>> aMatrix = {{1.5, 1.3}};
    // vector<vector<float>> bMatrix = {{1.3},{3}};

// /*(2x3)*/    vector<vector<float>> aMatrix = {{2.3, -4.5, 7.3}, {0, 1, -2}};
// /*(4x3)*/    vector<vector<float>> bMatrix = {{1.5, 2.7, 0}, {3, -4, 5.4}, {2.3, 1, -4.5}, {6.9, -5.5, 2}};
// /*(4x4)*/    vector<vector<float>> aMatrix = {{1.5, 2.7, 0, 5}, {3, -4, 5.4, -8}, {2.3, 1, -4.5, 9}, {6.9, -5.5, 2, 7}};
...
/*(6x6)*/    vector<vector<float>> aMatrix = {{1.5, 2.7, 0, 5, -7.6, 8}, {3, -4, 0, 3.6, 5.4, -8}, {2.3, 1, 6, 8.2, -4.5, 9}, {6.9, -5.5, 0, -6.2, 2, 7}, {1.5, 0.3, 2.1, -0.9, 4.3, -1.7}, {1.2, 3.4, 2.5, 0.7, 4.1, 1.9}};
// /*(4x4)cov*/    vector<vector<float>> aMatrix = {{-1.05,-2.21,-0.01,1.08}, {-2.21,-5.17,0.37,-2.28}, {-0.01,0.37,-3.30,0.35}, {1.08,-2.28,0.35,-2.14}};
...
// /*(10x10)*/    vector<vector<float>> aMatrix = {{2.5, 7.6, 2.3, -1.3, 0.7, -4.8, 3.1, 0, -6, -0.5}, {3.8, 0.2, -7.4, 5.6, -2.9, 6, 4, 8, -1.3, 8.5}, {4.5, 7.8, -2.3, -6.7, 1.2, 5.6, 3.4, -9, 0.1, 8}, {4.3, -7.5, 9.8, 2.5, -5.7, 8.9, 6.3, -3.1, 1.5, 0.6}, {-1, 3.2, 6.6, -8.8, 10, 2.2, 5.5, -7.7, 0, 4.4}, {8.7, -6.3, 4.9, 1.3, -5, 2.7, 0.9, 9.2, -7, 3}, {3.8, 0, 7.4, -5.6, 2.9, 9, 6.8, -4.7, 8.2, 1.3}, {2.4, 1.2, -3.6, 5.8, 4.1, 9.7, 6.5, -8.9, 7.3, -0.9}, {1.5, 2.7, 0, 5, -7.6, 8, 3, -4, 0, -8}, {2.3, 1, 6, 8.2, -4.5, 9, 6.9, 0, -6.2, 2}};
    ...
    // plainres = DomEigenvector_QR(aMatrix, number_of_iteration);
    ...
    // newDomEigenvector_QR(res, cipher1, number_of_iteration, length, bk);
    ...
    // practice(res, cipher1, cipher2, length, bk);

    // newPCAMatrix_step3_4_reducedbasis_decryptversion(res, cipher1, number_of_iteration, dimension, length, bk);
